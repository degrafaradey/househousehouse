...
КРИТИЧЕСКИ ВАЖНО про поле "narrator" — РОЛЬ СТРОГО ОГРАНИЧЕНА:
- Ты НЕ собеседник и не чат-бот. Ты НИКОГДА не здороваешься с игроком, не обращаешься к нему
  напрямую, не благодаришь, не отвечаешь на его вопросы от своего лица, не ведёшь светскую
  беседу. Единственная функция narrator — выдавать объективные игровые сведения (карточка
  пациента / история жителя в первом ходу; результат конкретного запрошенного "анализа";
  короткая механическая реакция экономики на действие игрока). Если игрок задаёт вопрос
  консультантам — на него отвечают ОНИ в "reactions", а не ты. Ты не участник диалога, ты
  система вывода информации.
- В ПЕРВОМ ходу игры лимит длины НЕ действует — это атмосферный многоабзацный рассказ от
  первого лица случайного ЖИТЕЛЯ/ПАЦИЕНТА страны (в духе истории болезни): кто он, чем
  занимается, что странного и труднообъяснимого он наблюдает, что ему говорят чиновники/
  начальство/родители — бытовым языком, с деталями и нестыковками. НЕ включай сюда сухой
  список объективных симптомов — они уходят отдельно в поле "symptoms" на доску, здесь только
  рассказ. ЭТОТ рассказ — от лица ПЕРСОНАЖА мира игры, не обращение к игроку.
- Во ВСЕХ последующих ходах — максимум 2 предложения, только факты, без обращения к игроку.
  Финальное раскрытие диагноза (case_solved=true) — тоже без лимита длины, но по-прежнему
  фактическое изложение, не светская речь.

ВАЖНО про игрока как реального человека: игрок — реальный человек за пределами вымышленного
мира кейса, не персонаж истории. Если игрок в переписке называет своё настоящее имя или другие
факты о себе — это информация О НЁМ, а не часть данных кейса (страны, персонажей, экономики).
Никогда не путай имя игрока с именами персонажей или названиями из кейса.

ВАЖНО про формат JSON — технические требования, иначе игра сломается:
- Внутри строковых значений JSON НИКОГДА не используй настоящий перенос строки — только
  экранированную последовательность \\n. Абзацы в атмосферном рассказе разделяй через \\n\\n
  (как текст), а не реальным Enter.
  - Не используй символ обычной прямой двойной кавычки " внутри текста реплик — для прямой
  речи персонажей используй ёлочки « » или обычные тире, но не ".
...
кейс — тогда допустим пустой список). Никогда не пропускай персонажа. Если игрок задал вопрос —
отвечать на него должны ОНИ здесь, а не narrator.

КРИТИЧЕСКИ ВАЖНО про споры команды: НЕ спорьте ради спора. Если тезис или наблюдение игрока по
существу верны — команда должна в основном СОГЛАШАТЬСЯ и развивать мысль дальше, а не искать
повод возразить. Настоящие разногласия — только там, где есть реальная почва (разные школы
экономической мысли, неоднозначные данные). Если diagnosis_check уже "correct_with_gaps" или
"solved" — реакции команды должны быть в основном одобрительными с точечными уточнениями, а НЕ
переспором с нуля. Игрок часто рассуждает по шагам, разбирая рассказ жителя на конкретные
"аномалии" и проверяя гипотезы одну за другой — цени и поддерживай такой подход, давай команде
реагировать на конкретные названные аномалии, а не спорить абстрактно.
...
2. При самом первом ходе — атмосферный рассказ жителя (см. правило narrator выше) И список из
   5-8 конкретных объективных СИМПТОМОВ в поле "symptoms" (короткие пункты для доски, например
   "ВВП: −6% за два года"). ТАКЖЕ в первом ходу заполни "gdp_trend" — динамику роста ВВП (% год
   к году) за последние 3-5 периодов, ведущую к текущему кризису, в формате списка объектов
   {{"period": "3 года назад", "value": 2.1}} ... {{"period": "сейчас", "value": -6.0}}. Числа
   должны логически соответствовать карточке пациента.
3. Реагировать на "тесты"/"лечение" игрока — через narrator (максимум 2 предложения, кроме
   первого хода и финала).
4. Team-реакции: 5-8 предложений на реплику каждого из трёх, с учётом правила про споры выше.
...
- Игрок — реальный человек за пределами вымышленного мира кейса. Если он называет своё
  настоящее имя или другие факты о себе — это информация о НЁМ, а не часть данных кейса.
  Не путай имя игрока с именами персонажей или названиями из истории кейса.
- Отвечай на русском."""
...
def _escape_newlines_in_json_strings(s: str) -> str:
    """LLM часто оставляет 'живые' переносы строк внутри строковых значений JSON,
    что ломает парсинг. Проходим посимвольно и чиним только то, что внутри строк."""
    out = []
    in_string = False
    escape = False
    for ch in s:
...
def render_whiteboard():
    ...
    solved = sum(st.session_state.symptom_status)
    rows = ""
    for i, s in enumerate(st.session_state.case_symptoms):
        ok = i < len(st.session_state.symptom_status) and st.session_state.symptom_status[i]
        mark = "✅" if ok else "▫️"
        weight = "600" if ok else "400"
        rows += (f'<p style="margin:4px 0;font-size:13px;color:#1a1a1a;font-weight:{weight};">'
                 f'{mark} {s}</p>')
    st.markdown(
        f'<div style="background:#ffffff;border:1px solid #ddd;border-radius:10px;'
        f'padding:14px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);">'
        f'<p style="font-size:13px;font-weight:700;color:#111;margin:0 0 10px;">'
        f'📋 ДОСКА · {solved}/{total} подтверждено</p>{rows}</div>',
        unsafe_allow_html=True,
    )
    if st.session_state.get("gdp_trend"):
        st.markdown('<div style="height:24px;"></div>', unsafe_allow_html=True)
        try:
            df = pd.DataFrame(st.session_state.gdp_trend).set_index("period")
            st.markdown(
                '<div style="background:#ffffff;border:1px solid #ddd;border-radius:10px;'
                'padding:14px 16px 6px;box-shadow:0 1px 4px rgba(0,0,0,0.08);">'
                '<p style="font-size:13px;font-weight:700;color:#111;margin:0 0 8px;">'
                '📈 Динамика ВВП, %</p></div>',
                unsafe_allow_html=True,
            )
    ...
    if st.button(f"💡 Подсказка ({hints_left})", disabled=hints_left <= 0, use_container_width=True):
        with st.spinner("Думаем..."):
            try:
                hint = get_hint(st.session_state.get("model_choice_value", "deepseek-chat"))
...
def get_main_reply(model: str) -> str:
    """Вызывает API. Если это не самый первый ход и команда промолчала (меньше 3 реплик),
    делает ОДНУ повторную попытку с явным напоминанием о формате."""
    reply = call_deepseek(st.session_state.messages, model)
    is_first_turn = not st.session_state.case_symptoms
    if not is_first_turn:
        data = parse_main_response(reply)
        if len(data.get("reactions", [])) < 3:
            reminder = {
                "role": "user",
                "content": ("[СИСТЕМА] В прошлом ответе не хватило реплик команды. Повтори "
                            "этот же ход, верни валидный JSON с РОВНО 3 объектами в "
                            "'reactions' — forman, cameron, chase, без исключений."),
            }
            try:
                retry_reply = call_deepseek(st.session_state.messages + [reminder], model)
                retry_data = parse_main_response(retry_reply)
                if len(retry_data.get("reactions", [])) >= 3:
                    reply = retry_reply
            except Exception:
                pass
    return reply
    ...
    if data["symptoms"]:
        ...
        if data.get("gdp_trend"):
            st.session_state.gdp_trend = data["gdp_trend"]
    ...
    if data.get("case_solved") and not st.session_state.current_case_archived:
        st.session_state.case_history.insert(0, {
            ...
            "gdp_trend": st.session_state.get("gdp_trend", []),
...
with st.sidebar:
    ...
    st.session_state.model_choice_value = model_choice
    st.divider()
...
# ---------------------------------------------------------------------------
# Доска симптомов (справа) + чат (слева)
# ---------------------------------------------------------------------------

main_col, board_col = st.columns([3, 2])
                ...
                if data.get("case_solved"):
                    ...
                    if c1.button("✅ Завершить", key=f"finalize_{idx}"):
                        ...
                        with st.spinner("Подводим итоги..."):
                            try:
                                reply = get_main_reply(model_choice)
                                apply_main_response(reply)
                                st.rerun()
                            except Exception as e:
                                st.error(f"Ошибка: {e}")
