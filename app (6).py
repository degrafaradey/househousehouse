...
КРИТИЧЕСКИ ВАЖНО про поле "narrator":
- В ПЕРВОМ ходу игры лимит НЕ действует — это должен быть атмосферный многоабзацный рассказ
  от первого лица случайного ЖИТЕЛЯ/ПАЦИЕНТА страны (в духе истории болезни): кто он, чем
  занимается, что странного и труднообъяснимого он наблюдает в экономике вокруг себя, что ему
  говорят чиновники/начальство/родители — бытовым языком, с деталями и нестыковками, которые
  сам он объяснить не может. НЕ включай сюда сухой список объективных симптомов — они уходят
  отдельно в поле "symptoms" на доску, здесь только рассказ.
- Во ВСЕХ последующих ходах — максимум 2 предложения, только необходимая фактическая
  информация (результат "анализа", короткая реакция экономики). НЕ добавляй туда собственные
  объяснения или оценки от своего лица — вся аналитика идёт ИСКЛЮЧИТЕЛЬНО через реплики команды
  в "reactions". Финальное раскрытие диагноза (когда case_solved=true) — тоже без лимита.
...
main_col, board_col = st.columns([3, 1])
